...
class DraggableImage : AppCompatImageView, View.OnTouchListener,
    ...
    private fun sharedConstructing(context: Context) {
        // Initialize the environment for the zoom app
    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            ...
            // Over the max scale factor
            if (mSaveScale > mMaxScale) {
                mSaveScale = mMaxScale
                mScaleFactor = mMaxScale / prevScale

                // Under the min scale factor
            } else if (mSaveScale < mMinScale) {
                mSaveScale = mMinScale
                mScaleFactor = mMinScale / prevScale
            }

            // To take them back at the center of the screen
    ...
    fun fixTranslation() {
        // To avoid going far away from the screen borders
    ...
    private fun getFixTranslation(trans: Float, viewSize: Float, contentSize: Float): Float {
        ...
        // To avoid going too far from the screen when moving
        if (contentSize <= viewSize) { // case: NOT ZOOMED
            minTrans = -contentSize
            maxTrans = viewSize
        } else { //CASE: ZOOMED
            minTrans = -contentSize
            maxTrans = viewSize
    ...
    */
    ...
    override fun onTouch(view: View?, event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (mode == DRAG) { // Move on the screen
                val dx = currentPoint.x - mLast.x
                val dy = currentPoint.y - mLast.y
                val fixTransX = dx //getFixDragTrans(dx, viewWidth.toFloat(), origWidth * mSaveScale)
                val fixTransY = dy //getFixDragTrans(dy, viewHeight.toFloat(), origHeight * mSaveScale)
            ...
            MotionEvent.ACTION_POINTER_UP -> mode = NONE // Leave the screen
        }
